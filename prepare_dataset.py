...
def changeGdf2Mat(dir_path, mode="train"):
    ...
    for nSub in range(1, 10):
        # Load the gdf file
        data_filename = dir_path+'BciCompetitionIv2a/{}/A0{}{}.gdf'.format(isTrain, nSub, mode_str)
        raw = mne.io.read_raw_gdf(data_filename, preload=True)  
        raw.filter(l_freq=8, h_freq=30, method='iir', iir_params=dict(order=5, ftype='butter'), phase='zero')
        ...
        epochs = mne.Epochs(raw, selected_events, event_id, picks=picks,tmin=0, tmax=4.028, preload=True, baseline=None)
        
        filtered_data = epochs.get_data()
        label_filename = 'BciCompetitionIv2a/' + 'true_labels/'+'A0{}{}.mat'.format(nSub, mode_str)
        mat = sio.loadmat(label_filename)  # load target mat file
        labels = mat['classlabel'] - 1
