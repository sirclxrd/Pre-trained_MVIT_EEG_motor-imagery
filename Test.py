...
from utils import (visualize_train_loss_acc, load_config, create_checkpoints_folders, 
                   append_to_log_file, load_only_model)
import random
import yaml
import argparse
...
subject_model = config["run"]["s_model"]
subject_test = config["run"]["s_test"]
batch_size = 32

#load_path = "Single_checkpoints/16_2_2/val_" + subject + ".pth"
load_path = "../../../mnt/localstorage/cdeangelis/Single_checkpoints/16_2_2/" + subject_model + ".pth"
...
test_dataset = prepare_dataloaders(subject_id = subject_test, onlytest = True, filter = config["train"]["filter"])
